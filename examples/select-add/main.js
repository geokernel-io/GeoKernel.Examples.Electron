...
const CITY_STYLE = Object.freeze({
  ...
  labelHaloWidth: 2,
  labelMinVisibleScale: 0,
  labelMaxVisibleScale: 0,
});
...
function shapeTypeName(shapeType) {
  const names = new Map([
    [1, "Point"],
    [3, "Polyline"],
    [5, "Polygon"],
  ...
  if (typeof shapeType === "string" && !/^\d+$/.test(shapeType)) return shapeType;
  return names.get(Number(shapeType)) ?? "Unknown";
}
  ...
  viewer.setLegendItems(
    selected.length > 0
      ? selected.map((hit, index) => ({
        shape: "none",
        label: `${index + 1} | ${hit.layerName ?? hit.layer ?? "-"} | ${hit.featureId} | ${shapeTypeName(hit.shapeType ?? hit.type)} | ${bestDisplayName(hit)}`,
      }))
...
function activateSelect() {
  ...
  viewer.setStatusText("Click = addSelectedFeature | Ctrl+Click = toggleSelectedFeature");
}
...
function handleCommand(commandId) {
  ...
  else if (commandId === COMMAND.FULL_EXTENT) viewer.setViewExtent(SAMPLE_EXTENT);
}
...
function selectAt(event) {
  ...
  const toggle = (modifiers & CONTROL_MODIFIER) !== 0;
  ...
  const operation = toggle ? "toggleSelectedFeature" : "addSelectedFeature";
  viewer.setStatusText(
    `${operation}: ${hit?.layerName ?? hit?.layer ?? "-"} feature ${hit?.featureId ?? "-"}`,
  );
}
...
function onViewerEvent(event) {
  ...
  if (event.eventType === ViewerEventType.SELECTION_CHANGED) {
    setImmediate(() => {
      if (viewer) updateSelectionPanel();
    });
  }
}
...
async function start() {
  ...
  viewer.addCommandToolbar([
    { id: COMMAND.SELECT, text: "Select Add" },
