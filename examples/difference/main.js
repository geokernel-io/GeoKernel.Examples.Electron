...
function renderScene(showResult) {
  ...
  if (!showResult) {
    details.push("Result: click Run Difference to calculate");
    viewer.setStatusText("Source polygons are ready. Click Run Difference.");
  } else {
    const parts = normalizeParts(viewer.differencePolygons(LEFT, RIGHT));
    if (parts.length === 0) {
      details.push("Result: empty");
      viewer.setStatusText("Difference returned an empty result.");
    } else {
      if (!viewer.addPolygonPartsShape(parts, RESULT_STYLE)) {
        throw new Error("Difference result could not be rendered.");
      }
      details.push(
        "Result type: polygon",
      ...
      viewer.setStatusText("Difference result created.");
    }
  }
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "Difference", width: 980, height: 680, navigationToolbar: false });
  viewer.addCommandToolbar([
    { id: 1, text: "Full Extent" },
    { id: 2, text: "Run Difference" },
  ], (id) => {
    try {
      if (id === 1) viewer.setViewExtent(FULL_EXTENT);
      else if (id === 2) renderScene(true);
    } catch (error) {
      viewer.setStatusText(`Difference failed: ${error.message}`);
      console.error(error?.stack || error);
    }
  });
  viewer.addLogPanel("Difference details");
