...
function activateSelect() {
  ...
  viewer.setStatusText("Click = add, Ctrl+Click = toggle. Watch selectionChanged log.");
}
...
function clearSelection() {
  ...
  viewer.setStatusText(`clearSelectedFeatures called. selectedFeatureCount=${count}.`);
}
...
function addSelectionAt(event) {
  ...
  const toggled = (modifiers & CONTROL_MODIFIER) !== 0;
  const changed = toggled
  ...
  const operation = toggled ? "toggleSelectedFeature" : "addSelectedFeature";
  viewer.setStatusText(`${operation} succeeded.`);
}

function appendSelectionSignal() {
  const count = viewer.selectedFeatureCount();
  signalSequence += 1;
  ...
  viewer.appendLog(`${String(signalSequence).padStart(3)} | ${timestamp} | selectionChanged | ${count}`);
  updateSelectionPanel();
  viewer.setStatusText(`selectionChanged(${count})`);
}
...
function onViewerEvent(event) {
  ...
  if (event.eventType === ViewerEventType.MAP_MOUSE_UP && currentTool === ViewerTool.INFO) {
    ...
    setImmediate(() => { if (viewer) appendSelectionSignal(); });
  }
}
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "SelectionSignal", width: 1200, height: 800, navigationToolbar: false });
