...
const COMMAND = Object.freeze({ IDENTIFY: 1, PAN: 2, FULL_EXTENT: 3 });
...
function extentText(value) {
  if (!value) return "-";
  const number = (candidate) => Number(candidate).toFixed(6);
  return `(${number(value.xMin)}, ${number(value.yMin)}) - (${number(value.xMax)}, ${number(value.yMax)})`;
}
...
function showEmptyHit() {
  viewer.clearLog();
  viewer.appendLog("Property / Field     Value");
  viewer.appendLog("Hit                  No feature at clicked point");
}
...
function activateIdentify() {
  ...
  viewer.setStatusText("Tool: hitTestTopFeatureAt. Click a feature to inspect the top-most hit.");
}
...
function inspectAt(event) {
  ...
  showHit(hit);
  viewer.setStatusText(`Top hit: ${hit.layerName ?? hit.layer ?? "-"} feature ${hit.featureId}.`);
}
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "ClickHitTest", width: 1200, height: 800, navigationToolbar: false });
  viewer.addCommandToolbar([
    { id: COMMAND.IDENTIFY, text: "Click Hit Test" },
    { id: COMMAND.PAN, text: "Pan" },
    { id: COMMAND.FULL_EXTENT, text: "Full Extent", separatorBefore: true },
  ], onCommand);
  viewer.addLogPanel("Top feature hit");
  viewer.setTool(ViewerTool.INFO);
