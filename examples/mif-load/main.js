...
const MIF_SAMPLE_URL = "https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/albania.zip";
const MIF_STYLE = Object.freeze({
  fillColor: "#D7E5DF",
  fillOpacity: 184,
  lineColor: "#6D8C86",
  lineWidth: 1.1,
...
function sidecarLine(mifPath, extension) {
  const candidate = path.join(path.dirname(mifPath), `${path.parse(mifPath).name}${extension}`);
...
function inspectionText(mifPath) {
  ...
  return [
    ...
    "",
    "Sidecars",
    sidecarLine(mifPath, ".mif"),
    sidecarLine(mifPath, ".mid"),
    "",
    "Attribute schema",
    ...schema,
    "",
...
async function start() {
  ...
  viewer.setStatusText("Preparing MapInfo MIF sample data...");
  viewer.show();
  viewer.processEvents();
  startEventPump();

  const mifPath = await ensureSampleFile(
    MIF_SAMPLE_URL,
    "albania.zip",
    "albania",
    "albania.mif",
  ...
  viewer.addLayer(mifPath);
  viewer.setLayerName(0, "Albania MIF");
  viewer.setLayerStyle(0, MIF_STYLE);
  viewer.clearLog();
  viewer.appendLog(inspectionText(mifPath));
  viewer.zoomToLayer(0);
  viewer.setStatusText(
    `MifLoad opened ${viewer.layerFeatureCount(0)} features and `
