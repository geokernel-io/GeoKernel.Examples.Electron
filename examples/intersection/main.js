...
const LEFT = [[-4.2, -1.7], [1.0, -1.7], [1.0, 2.1], [-4.2, 2.1], [-4.2, -1.7]];
const RIGHT = [
...
const LEFT_STYLE = { fillColor: "#BFD7EA", fillOpacity: 120, lineColor: "#2F80C2", lineWidth: 2.0 };
const RIGHT_STYLE = { fillColor: "#CDE7D8", fillOpacity: 120, lineColor: "#2D6A4F", lineWidth: 2.0 };
const RESULT_STYLE = { fillColor: "#F9C74F", fillOpacity: 165, lineColor: "#D95D39", lineWidth: 3.0 };
...
function renderScene(showResult) {
  ...
  if (!showResult) {
    details.push("Result: click Run Intersection to calculate");
    viewer.setStatusText("Source polygons are ready. Click Run Intersection.");
  } else {
    const parts = normalizeParts(viewer.intersectionPolygons(LEFT, RIGHT));
    if (parts.length === 0) {
      details.push("Result: empty");
      viewer.setStatusText("Intersection returned an empty result.");
    } else {
      for (const part of parts) {
        if (!viewer.addPolygonShape(part, RESULT_STYLE)) throw new Error("Intersection result shape could not be rendered.");
      }
      details.push(
        "Result type: polygon",
      ...
      viewer.setStatusText("Intersection result created.");
    }
  }
  viewer.clearLog();
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "Intersection", width: 980, height: 680, navigationToolbar: false });
  viewer.addCommandToolbar([
    { id: 1, text: "Full Extent" },
    { id: 2, text: "Run Intersection" },
  ], (id) => {
    try {
      if (id === 1) viewer.setViewExtent(FULL_EXTENT);
      else if (id === 2) renderScene(true);
    } catch (error) {
      viewer.setStatusText(`Intersection failed: ${error.message}`);
      console.error(error?.stack || error);
    }
  });
  viewer.addLogPanel("Intersection details");
