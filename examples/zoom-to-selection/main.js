...
const COMMAND = Object.freeze({ ADD: 1, TOGGLE: 2, PAN: 3, CLEAR: 4, ZOOM: 5, FULL_EXTENT: 6 });
...
function selectedExtent(selected) {
  const extents = selected.map((hit) => hit.extent).filter(Boolean);
  if (extents.length === 0) return null;
  return {
    xMin: Math.min(...extents.map((value) => Number(value.xMin))),
    yMin: Math.min(...extents.map((value) => Number(value.yMin))),
    xMax: Math.max(...extents.map((value) => Number(value.xMax))),
    yMax: Math.max(...extents.map((value) => Number(value.yMax))),
  };
}

function extentText(value) {
  if (!value) return "<empty>";
  return `(${value.xMin.toFixed(4)}, ${value.yMin.toFixed(4)}) - (${value.xMax.toFixed(4)}, ${value.yMax.toFixed(4)})`;
}
...
function updateSelectionPanel(emptyMessage = "No selected features.") {
  ...
  const selectedBounds = selectedExtent(selected);
  ...
  return { selected, selectedBounds };
}
...
function activateSelectionMode(mode) {
  ...
  viewer.setStatusText(mode === "toggle"
    ? "Click = toggleSelectedFeature. Click a feature to toggle it in the selection."
    : "Click = addSelectedFeature. Click a feature to add it to the selection.");
}
...
function clearSelection() {
  ...
  updateSelectionPanel("Selection cleared.");
  viewer.setStatusText("Selection cleared.");
}

function zoomToSelection() {
  const { selected, selectedBounds } = updateSelectionPanel();
  if (selected.length === 0) {
    viewer.setStatusText("No selected features to zoom.");
    return;
  }
  if (!viewer.zoomToSelectedFeatures()) {
    viewer.setStatusText("No selected feature extent to zoom.");
    return;
  }
  viewer.setStatusText(`zoomToSelectedFeatures ok. selectedFeaturesExtent=${extentText(selectedBounds)}.`);
}

function handleCommand(commandId) {
  if (commandId === COMMAND.ADD) activateSelectionMode("add");
  else if (commandId === COMMAND.TOGGLE) activateSelectionMode("toggle");
  else if (commandId === COMMAND.PAN) activatePan();
  else if (commandId === COMMAND.CLEAR) clearSelection();
  else if (commandId === COMMAND.ZOOM) zoomToSelection();
...
function selectAt(event) {
  ...
  const changed = selectionMode === "toggle"
    ? viewer.toggleTopFeatureSelectionAt(x, y, 8)
    : viewer.addTopFeatureToSelectionAt(x, y, 8);
  if (!changed) {
    viewer.setStatusText("No feature hit. Existing selection was preserved.");
    return;
  }
  const { selected, selectedBounds } = updateSelectionPanel();
  viewer.setStatusText(`${selectionMode === "toggle" ? "toggleSelectedFeature" : "addSelectedFeature"} applied. Selected: ${selected.length} | selectedFeaturesExtent: ${extentText(selectedBounds)}.`);
}
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "ZoomToSelection", width: 1200, height: 800, navigationToolbar: false });
  viewer.addCommandToolbar([
    { id: COMMAND.ADD, text: "Add Selection" },
    { id: COMMAND.TOGGLE, text: "Toggle Selection" },
    { id: COMMAND.PAN, text: "Pan" },
    { id: COMMAND.CLEAR, text: "Clear Selection", separatorBefore: true },
    { id: COMMAND.ZOOM, text: "zoomToSelectedFeatures" },
