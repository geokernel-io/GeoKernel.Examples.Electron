...
let signalHistory = [];
let detailMessage = "Drag a selection box to list matching features.";
...
function selectionMode(modifiers) {
  if ((modifiers & CONTROL_MODIFIER) !== 0) return 2;
  if ((modifiers & SHIFT_MODIFIER) !== 0) return 1;
  return 0;
}

function rectText(left, top, right, bottom) {
...
function renderLog(detailHit = null) {
  viewer.clearLog();
  viewer.appendLog("mapSelectionBoxFinished(rect, extent, modifiers)");
  viewer.appendLog("Time         | Screen rect | World extent | Modifiers | Hit count");
  for (const entry of signalHistory) viewer.appendLog(entry);
  viewer.appendLog("");
  viewer.appendLog("Selected hit details");
  if (!detailHit) {
    viewer.appendLog(`Info                 ${detailMessage ?? "Select a result row."}`);
    return;
  }
  const rows = [
    ["Layer", detailHit.layerName ?? detailHit.layer ?? "-"],
    ["Layer index", detailHit.layerIndex],
    ["Shape id", detailHit.shapeId],
    ["Feature id", detailHit.featureId],
    ["Shape type", detailHit.shapeType ?? detailHit.type ?? "-"],
    ["Extent", extentText(detailHit.extent)],
  ];
  for (const key of Object.keys(detailHit.attributes ?? {}).sort((left, right) => left.localeCompare(right))) {
    rows.push([key, detailHit.attributes[key]]);
  }
...
function activateSelect() {
  ...
  viewer.setStatusText("Drag a box to emit MAP_SELECTION_BOX_FINISHED(rect, extent, modifiers).");
}
...
function handleSelectionBox(event) {
  ...
  const boxHits = viewer.hitTestFeaturesInScreenRect(left, top, right, bottom).filter((hit) => hit?.isValid);
  viewer.selectFeaturesInScreenRect(left, top, right, bottom, selectionMode(modifiers));
  ...
  signalHistory.push(
    `${timestamp} | ${rectText(left, top, right, bottom)} | ${extentText(event.extent)} | ${modifiersText(modifiers)} | ${boxHits.length}`
  );
  updateHitsPanel();
  ...
  viewer.setStatusText(
    `MAP_SELECTION_BOX_FINISHED: ${rectText(left, top, right, bottom)} extent=${extentText(event.extent)} modifiers=${modifiersText(modifiers)} hits=${boxHits.length}`
  );
}
...
async function start() {
  ...
  viewer = new ViewerWindow({ title: "SelectionBoxSignal", width: 1200, height: 800, navigationToolbar: false });
  ...
  viewer.addLogPanel("MAP_SELECTION_BOX_FINISHED log and selected hit details");
  viewer.setTool(ViewerTool.SELECT);
...
function stop() {
  ...
  currentHitIndex = -1;
  signalHistory = [];
  detailMessage = "Drag a selection box to list matching features.";
